...
to get a speedup over using CGI.pm, don't 'use warnings;' or 'use vars'
...
execution environment). While you can reach around 50 or so invokations per second on
...
using CGI.pm and CGI::Minimal to read a passed parameter and print it
and also a 'null' Perl script which didn't actually read the passed cgi parameter
but printed the same result as if it had.

With 10 parallel fetches repeated as fast as possible for 20 seconds 
with and without mod_perl the tests gave the following:
  
  CGI.pm       via standard CGI -  15 fetches per second
  CGI::Minimal via standard CGI -  57 fetches per second
  null script  via standard CGI - 103 fetches per second
  CGI.pm       via mod_perl     - 361 fetches per second
  CGI::Minimal via mod_perl     - 416 fetches per second
  null script  via mod_perl     - 500 fetches per second
...
to provide SGML/XML compatibility for those needing it. Since ';' has
...
Version 1.14 16 Oct 2004

=head1 COPYRIGHT

Copyright (c) Benjamin Franz 1999-2004. All rights reserved.

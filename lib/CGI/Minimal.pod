...
execution environment). While you can reach around 67 or so invokations per second on
...
option slowed the CGI::Minimal script down from 67 fetches/second 
to only 53 fetches/second.

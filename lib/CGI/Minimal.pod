...
execution environment). While you can reach around 70 or so invokations per second on
...
using CGI.pm, CGI::Lite, CGI::Deurl, cgi-lib.pl, CGI::Thin
...
didn't actually read the passed cgi parameter but printed the same results as if they had.

Notably, mod_perl outperforms the compiled C program by
quite a lot for this simple case.
...
results (sorted by speed):
  ...
  CGI::Minimal (1.15) via standard CGI -  70 fetches per second
  ...
  CGI::Minimal (1.15) via mod_perl     - 417 fetches per second
